...
proc toJS*(ctx: JSContext; x: HintItem): JSValue =
  ...
  if JS_IsException(obj):
    return JS_EXCEPTION
  block good:
    if ctx.definePropertyCWE(obj, "url", ctx.toJS(x.url)) == dprException:
      break good
...
proc hideHints*(bc: BufferContext) {.proxy.} =
  for element in bc.window.document.elementDescendants(TAG_A):
    HTMLAnchorElement(element).setHint(false)
  bc.maybeReshape()
