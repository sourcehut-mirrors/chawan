...
proc getCacheFile(pager: Pager; cacheId: int; pid = -1): string
proc getTempFile(pager: Pager; ext = ""): string
proc connected2(pager: Pager; init: BufferInit): JSValue
proc findMailcapPrevNext(pager: Pager; init: BufferInit; i: int):
  tuple[prev, next: int]
proc copyLoadInfo(pager: Pager; init: BufferInit)
proc unregisterBufferIface(pager: Pager; iface: BufferInterface)
...
proc newPager*(config: Config; forkserver: ForkServer; ctx: JSContext;
  ...
  let showConsole = ctx.eval("Pager.prototype.showConsole", "<init>",
    JS_EVAL_TYPE_GLOBAL)
  if JS_IsException(handleInput.vc) or JS_IsException(showConsole.vc):
    return Pager(nil)
  if not JS_IsFunction(ctx, handleInput.vc) or
      not JS_IsFunction(ctx, showConsole.vc):
  ...
  for sc in pager.config.siteconf:
    ...
    if sc.fun != nil:
      ...
      if JS_IsException(arg0.vc):
        ...
        let ret = ctx.callSink(sc.fun, JS_UNDEFINED.vc, arg0)
        if not JS_IsException(ret.vc):
...
proc runMailcapWriteFile(pager: Pager; stream: PosixStream;
    needsterminal: bool; cmd, outpath: string): Opt[void] =
...
#TODO add support for edit/compose
proc runMailcap(pager: Pager; init: BufferInit; entry: MailcapEntry):
    Opt[void] =
  let url = init.url
  ...
  if nametemplate != nil:
    outpath = unquoteCommand(nametemplate.value, init.contentType, outpath,
      url)
  var canpipe = true
  let cmd = unquoteCommand(entry.cmd, init.contentType, outpath, url, canpipe)
...
proc saveEntry(pager: Pager; contentType: string; entry: MailcapEntry) =
  ...
  if pager.autoMailcap.saveEntry(path, contentType, entry).isErr:
    pager.alert("Could not write to " & $path)

proc initMailcap(pager: Pager; init: BufferInit): JSValue =
  let ctx = pager.jsctx
  # contentType must have been set by applyResponse.
  if init.shortContentType.startsWith("text/"):
    # prepare content type for %{charset}
    init.contentType.setContentTypeAttr("charset", $init.charset)
  let entry = if bifForceType in init.flags:
    pager.autoMailcap.findMailcapEntry(init.shortContentType, init.contentType,
      init.url)
  else:
    pager.autoMailcap.findMailcapEntryMut(init.shortContentType,
      init.contentType, init.url)
  if init.shortContentType == "text/html":
    init.flags.incl(bifHTML)
    return pager.connected2(init)
  if init.shortContentType == "text/plain" or bifSave in init.flags:
    return pager.connected2(init)
  if entry != nil:
...
jsClassDef(Pager):
  ...
  proc setLineEdit0(ctx: JSContext; pager: Pager; mode: LineMode;
      prompt: sink string; obj = JS_UNDEFINED.vc): JSValue {.jsfunc.} =
    var current = ""
    var hide = false
  ...
  proc omniRewrite(ctx: JSContext; pager: Pager; arg0: JSValueConst): JSValue
    ...
    for rule in pager.config.omnirule:
      if rule.regex.match(s):
        ...
        return ctx.call(rule.fun, JS_UNDEFINED.vc, arg0)
  ...
  proc applyMailcap(ctx: JSContext; pager: Pager; init: BufferInit;
    ...
    if JS_IsNumber(val):
      ...
      if res.isOk:
        ...
        return JS_ThrowTypeError(ctx, "%s", cstring(state.error))
    return JS_UNDEFINED
  ...
  proc findMailcapPrevNext(pager: Pager; init: BufferInit; i: int):
    ...
    let prev = pager.mailcap.findPrevMailcapEntry(init.shortContentType,
      init.contentType, init.url, i)
    var next = i
    discard pager.mailcap.findMailcapEntry(init.shortContentType,
      init.contentType, init.url, next)
    return (prev, next)
  ...
  # private
  proc inputLoop(ctx: JSContext; pager: Pager): Opt[void] {.jsfunc.} =
