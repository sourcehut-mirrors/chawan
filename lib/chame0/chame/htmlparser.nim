...
type
  HTML5ParserOpts*[Handle] = object
    ...
    scripting*: bool ## Is scripting enabled for this document?
      ...
      ## ctx != Handle.default, otherwise "Normal"/"Disabled" are assumed.
    ctxIsIntegrationPoint*: bool ## Must be set if ctx != Handle.default and
      ...
      ## desc, or title.
    ctx*: Handle ## Context element for fragment parsing.
      ## When set to some Handle, the fragment case is used while parsing.
    openElementsInit*: Handle ## Node to push to the stack of open
      ## elements.  This should be set to a new HTML element in fragment
      ## parsing mode, and left empty otherwise.
    formInit*: Handle ## Initial state of the parser's form pointer.
...
# 13.2.4.1
  InsertionMode = enum
    ...
    imInTableText, imInCaption, imInColumnGroup, imInTableBody, imInRow,
...
proc processInHTML[Handle, Atom](parser: var HTML5Parser[Handle, Atom]):
  ParseChunkResult
proc processBeforeHtml[Handle, Atom](parser: var HTML5Parser[Handle, Atom]):
  ParseChunkResult
proc processBeforeHead[Handle, Atom](parser: var HTML5Parser[Handle, Atom]):
  ParseChunkResult
proc processInHead[Handle, Atom](parser: var HTML5Parser[Handle, Atom]):
  ParseChunkResult
proc processAfterHead[Handle, Atom](parser: var HTML5Parser[Handle, Atom]):
  ParseChunkResult
...
proc processInTableText[Handle, Atom](parser: var HTML5Parser[Handle, Atom]):
  ParseChunkResult
proc processInColumnGroup[Handle, Atom](parser: var HTML5Parser[Handle, Atom]):
  ParseChunkResult
proc processInTableBody[Handle, Atom](parser: var HTML5Parser[Handle, Atom]):
  ParseChunkResult
proc processInRow[Handle, Atom](parser: var HTML5Parser[Handle, Atom]):
  ParseChunkResult
proc processAfterBody[Handle, Atom](parser: var HTML5Parser[Handle, Atom]):
  ParseChunkResult
...
proc hasContext[Handle, Atom](parser: HTML5Parser[Handle, Atom]): bool =
  parser.ctx.element != Handle.default
...
proc adjustedCurrentNodeToken[Handle, Atom](parser: HTML5Parser[Handle, Atom]):
    OpenElement[Handle, Atom] =
  if parser.hasContext() and parser.openElements.len == 1:
    return parser.ctx
  else:
...
proc append[Handle, Atom](parser: HTML5Parser[Handle, Atom];
    parent, node: Handle) =
  parser.insertBefore(parent, node, Handle.default)
...
proc processBeforeHtml[Handle, Atom](parser: var HTML5Parser[Handle, Atom]):
  ...
  if anythingElse:
    ...
    parser.pushHTMLElement(element)
    parser.insertionMode = imBeforeHead
    return parser.processBeforeHead()
  pcrContinue
...
proc processBeforeHead[Handle, Atom](parser: var HTML5Parser[Handle, Atom]):
  ...
  of ttStartTag:
    ...
    of ttHtml:
      return parser.processInBody()
    of ttHead:
      parser.head = parser.insertHTMLElement()
  ...
  if anythingElse:
    parser.head = parser.insertHTMLElement(ttHead)
    parser.insertionMode = imInHead
    return parser.processInHead()
  pcrContinue
...
proc processInHead[Handle, Atom](parser: var HTML5Parser[Handle, Atom]):
  ...
  of ttStartTag:
    ...
    of ttScript:
      ...
      let element = parser.createHTMLElement(parser.tok.tagname, location.inside,
        move(parser.tok.attrs))
      if parser.hasContext() and not parser.scripting:
  ...
  if anythingElse:
    discard parser.popElement()
    parser.insertionMode = imAfterHead
    return parser.processAfterHead()
  pcrContinue

proc processInHeadNoscript[Handle, Atom](
...
proc processAfterHead[Handle, Atom](parser: var HTML5Parser[Handle, Atom]):
  ...
  of ttEndTag:
    case parser.toTagType(parser.tok.tagname)
    of ttTemplate: return parser.processInHead()
    of ttBody, ttHtml, ttBr: anythingElse = true
    else: discard
  else: anythingElse = true
  if anythingElse:
...
proc processInBody[Handle, Atom](parser: var HTML5Parser[Handle, Atom]):
  ...
  of ttStartTag:
    ...
    of ttForm:
      ...
      if parser.form == Handle.default or hasTemplate:
        parser.closeP()
    ...
    of ttArea, ttBr, ttEmbed, ttImg, ttKeygen, ttWbr:
      ...
      parser.framesetOk = false
    of ttInput:
      if not parser.hasContext() or
          parser.getTagType(parser.ctx.element) != ttSelect:
  ...
  of ttEndTag:
    ...
    of ttHtml:
      if parser.hasElementInScope(ttBody):
        parser.insertionMode = imAfterBody
        return parser.processAfterBody()
    ...
    of ttForm:
      if not parser.hasElement(ttTemplate):
        let form = move(parser.form)
        if form == Handle.default or not parser.hasElementInScope(form):
          return
        parser.generateImpliedEndTags()
        let i = parser.findOpenElement(form)
        parser.openElements.delete(i)
      else:
...
proc processLineFeedTrim[Handle, Atom](parser: var HTML5Parser[Handle, Atom]):
    ParseChunkResult =
  if parser.insertionMode == imPreStart:
...
proc processInTable[Handle, Atom](parser: var HTML5Parser[Handle, Atom]):
    ParseChunkResult =
  var anythingElse = false
  case parser.tok.t
  of ttCharacter, ttWhitespace, ttNull:
    ...
    if parser.getTagType(parser.currentNode) in CanHaveText:
      ...
      parser.oldInsertionMode = parser.insertionMode
      parser.insertionMode = imInTableText
      return parser.processInTableText()
    else:
      anythingElse = true
  of ttComment: parser.insertComment()
  of ttDoctype: discard
  of ttStartTag:
    ...
    of ttCol:
      ...
      discard parser.insertHTMLElement(ttColgroup)
      parser.insertionMode = imInColumnGroup
      return parser.processInColumnGroup()
    ...
    of ttTd, ttTh, ttTr:
      ...
      parser.insertionMode = imInTableBody
      return parser.processInTableBody()
    of ttTable:
    ...
    of ttInput:
      ...
      if i >= 0 and
        ...
        anythingElse = true
    of ttStyle, ttScript, ttTemplate: return parser.processInHead()
    of ttForm:
      if parser.form == Handle.default and not parser.hasElement(ttTemplate):
        parser.form = parser.insertHTMLElement()
        discard parser.popElement()
    else: anythingElse = true
  of ttEndTag:
    case parser.toTagType(parser.tok.tagname)
...
proc processInCaption[Handle, Atom](parser: var HTML5Parser[Handle, Atom]):
    ParseChunkResult =
  var anythingElse = false
  case parser.tok.t
  of ttStartTag:
  ...
  of ttStartTag:
    ...
    of ttCaption, ttCol, ttColgroup, ttTbody, ttTfoot, ttThead:
      if parser.hasElementInTableScope({ttTbody, ttThead, ttTfoot}):
        ...
        parser.insertionMode = imInTable
        return parser.processInTable()
    else: return parser.processInTable()
  of ttEndTag:
    case (let tokTagType = parser.toTagType(parser.tok.tagname); tokTagType)
...
proc processInCell[Handle, Atom](parser: var HTML5Parser[Handle, Atom]):
    ParseChunkResult =
  case parser.tok.t
  of ttStartTag:
    case parser.toTagType(parser.tok.tagname)
    of ttCaption, ttCol, ttColgroup, ttTbody, ttTd, ttTfoot,
      ...
      if parser.hasElementInTableScope({ttTd, ttTh}):
        parser.closeCell()
        return parser.processInRow()
    else: return parser.processInBody()
  of ttEndTag:
    case (let tokTagType = parser.toTagType(parser.tok.tagname); tokTagType)
    of ttTd, ttTh:
      if parser.hasElementInTableScope(tokTagType):
...
proc processInTemplate[Handle, Atom](parser: var HTML5Parser[Handle, Atom]):
    ParseChunkResult =
  case parser.tok.t
...
proc processAfterBody[Handle, Atom](parser: var HTML5Parser[Handle, Atom]):
    ParseChunkResult =
  case parser.tok.t
  of ttWhitespace: return parser.processInBody()
...
proc processAfterAfterBody[Handle, Atom](
    parser: var HTML5Parser[Handle, Atom]): ParseChunkResult =
  case parser.tok.t
  of ttComment: parser.insertComment(lastChildOf(parser.getDocument()))
...
proc processAfterAfterFrameset[Handle, Atom](
    parser: var HTML5Parser[Handle, Atom]): ParseChunkResult =
  case parser.tok.t
  of ttComment: parser.insertComment(lastChildOf(parser.getDocument()))
  of ttDoctype, ttWhitespace: return parser.processInBody()
...
proc processInHTML[Handle, Atom](parser: var HTML5Parser[Handle, Atom]):
    ParseChunkResult =
  return case parser.insertionMode
  of imInitial: parser.processInitial()
  of imBeforeHtml: parser.processBeforeHtml()
  of imBeforeHead: parser.processBeforeHead()
  of imInHead: parser.processInHead()
  of imInHeadNoscript: parser.processInHeadNoscript()
  of imAfterHead: parser.processAfterHead()
  of imInBody: parser.processInBody()
  of imPreStart, imTextareaStart: parser.processLineFeedTrim()
  of imText: parser.processText()
  of imInTable: parser.processInTable()
  of imInTableText: parser.processInTableText()
  of imInCaption: parser.processInCaption()
  of imInColumnGroup: parser.processInColumnGroup()
  of imInTableBody: parser.processInTableBody()
  of imInRow: parser.processInRow()
  of imInCell: parser.processInCell()
  of imInTemplate: parser.processInTemplate()
  of imAfterBody: parser.processAfterBody()
  of imInFrameset: parser.processInFrameset()
  of imAfterFrameset: parser.processAfterFrameset()
  of imAfterAfterBody: parser.processAfterAfterBody()
  of imAfterAfterFrameset: parser.processAfterAfterFrameset()
...
proc initHTML5Parser*[Handle, Atom](dombuilder: DOMBuilder[Handle, Atom];
  ...
  if opts.ctx != Handle.default:
    let ctxInit = opts.ctx
